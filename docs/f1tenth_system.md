...
Documentation from [f1tenth/Robotacer's f1tenth_system repository](https://github.com/f1tenth/f1tenth_system) which contains the drivers and system level code including the driver stack. 
The driver stack provides the hardware interface between ROS and the F1TENTH car, reading sensor data (e.g., VESC speed, steering feedback) and sending actuator commands. 
It converts ROS control messages (like steering and throttle) into low-level VESC motor and servo commands that physically drive the car. 
Driver stack was implemented from [RoboRacer documentation](https://f1tenth.readthedocs.io/en/foxy_test/getting_started/firmware/drive_workspace_docker.html#doc-drive-workspace-docker) 
