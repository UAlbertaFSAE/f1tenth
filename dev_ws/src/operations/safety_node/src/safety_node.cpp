...
class Safety : public rclcpp::Node {
  // automatic emergency braking node that uses 2D laserscans and time of flight calculations
 ...
 private:
  ...
  void odom_callback(const nav_msgs::msg::Odometry::ConstSharedPtr msg) {
    ...
    RCLCPP_INFO(this->get_logger(), "current speed: '%f'", this->speed);
  }
  ...
  void scan_callback(const sensor_msgs::msg::LaserScan::ConstSharedPtr scan_msg) {
    ...
    if (emergency_breaking) {
      ...
      RCLCPP_INFO(this->get_logger(), "emergency brake engaged at speed '%f'",
                  this->speed);  // Output to log;
