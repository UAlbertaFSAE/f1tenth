...
WaypointGenerator::WaypointGenerator() : Node("waypoint_generator_node") {
  ...
  this->declare_parameter(
      "save_path", "/f1tenth/dev_ws/src/navigation/waypoint_generator/src/waypoints_odom.csv");
...
void WaypointGenerator::odom_callback(
    const nav_msgs::msg::Odometry::ConstSharedPtr odom_submsgObj) {
  csv_odom.open(save_path, std::ios::out | std::ios::app);

  double diff = sqrt(pow((odom_submsgObj->pose.pose.position.x - x_old), 2) +
                     pow((odom_submsgObj->pose.pose.position.y - y_old), 2));
  ...
  if (diff > min_distance) {
    ...
    RCLCPP_INFO(this->get_logger(), "%f....%f", odom_submsgObj->pose.pose.position.x,
                odom_submsgObj->pose.pose.position.y);
