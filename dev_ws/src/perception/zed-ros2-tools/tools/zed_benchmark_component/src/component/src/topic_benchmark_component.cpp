...
template <typename T>
void TopicBenchmarkComponent::getParam(std::string paramName, T defValue, T& outVal,
...
void TopicBenchmarkComponent::updateTopicInfo() {
    ...
    if (topic_name == mTopicName) {
      ...
      for (const auto& topic_type : topicTypes) {
        ...
        RCLCPP_INFO_STREAM(get_logger(),
                           "Found topic: '" << mTopicName << "' of type: '" << topic_type << "'");
  ...
  if (!mTopicAvailable.load()) {
    RCLCPP_INFO_STREAM_ONCE(get_logger(),
                            "Waiting for topic '" << mTopicName << "' to be published...");
  } else {
    if (mTopicTimer) {
      mTopicTimer->cancel();
    }
