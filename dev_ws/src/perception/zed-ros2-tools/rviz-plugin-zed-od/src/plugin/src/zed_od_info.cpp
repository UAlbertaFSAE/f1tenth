...
ZedOdInfo::ZedOdInfo(zed_interfaces::msg::Object& obj, Ogre::SceneManager* scene_manager,
...
void ZedOdInfo::updateInfo(zed_interfaces::msg::Object& obj) {
  ...
  if (create) {
    ...
    mLabel->setTextAlignment(rviz_rendering::MovableText::H_CENTER,
                             rviz_rendering::MovableText::V_CENTER);

    mPivot = std::make_shared<rviz_rendering::Shape>(rviz_rendering::Shape::Sphere, mSceneManager,
                                                     mPivotSceneNode);
  ...
  for (int i = 0; i < 8; i++) {
    ...
    shapePtr sphere;
    if (create) {
      sphere = std::make_shared<rviz_rendering::Shape>(rviz_rendering::Shape::Sphere, mSceneManager,
                                                       mBBoxSceneNode);
      Ogre::Vector3 scale;
  ...
  if (obj.skeleton_available) {
    ...
    for (int i = 0; i < size; i++) {
      ...
      shapePtr sphere;
      if (create) {
        sphere = std::make_shared<rviz_rendering::Shape>(rviz_rendering::Shape::Sphere,
      ...
      if (qIsNaN(pos[0])) {
        ...
        sphere->setScale(Ogre::Vector3(mJointRadius * mSkelScale, mJointRadius * mSkelScale,
