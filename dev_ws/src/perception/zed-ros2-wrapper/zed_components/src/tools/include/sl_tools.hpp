 ...
 */
std::unique_ptr<sensor_msgs::msg::Image> imageToROSmsg(sl::Mat& img, std::string frameId,
                                                       rclcpp::Time t);
 ...
 */
std::unique_ptr<sensor_msgs::msg::Image> imagesToROSmsg(sl::Mat& left, sl::Mat& right,
...
std::vector<std::vector<float>> parseStringVector(const std::string& input,
...
class StopWatch {
 public:
  ...
  double toc(std::string func_name =
                 std::string());  //!< Returns the seconds elapsed from the last tic in ROS clock
                                  //!< reference (it works also in simulation)
