  ...
  if (ZED_SDK_MAJOR_VERSION < SDK_MAJOR_REQ ||
    ...
    RCLCPP_ERROR_STREAM(get_logger(),
                        "This version of the ZED ROS2 wrapper is designed to work with ZED SDK v"
                            << static_cast<int>(SDK_MAJOR_REQ) << "."
                            << static_cast<int>(SDK_MINOR_REQ) << " or newer.");
    RCLCPP_INFO_STREAM(get_logger(), "* Detected SDK v" << ZED_SDK_MAJOR_VERSION << "."
...
std::string ZedCamera::getParam(std::string paramName, std::vector<std::vector<float>> &outVal) {
  ...
  if (error != "") {
    RCLCPP_WARN_STREAM(get_logger(),
                       "Error parsing " << paramName << " parameter: " << error.c_str());
...
std::string ZedCamera::parseRoiPoly(const std::vector<std::vector<float>> &in_poly,
...
void ZedCamera::getDebugParams() {
  ...
  RCLCPP_INFO(get_logger(), " * Debug Object Detection: %s", mDebugObjectDet ? "TRUE" : "FALSE");

  getParam("debug.debug_body_tracking", mDebugBodyTrk, mDebugBodyTrk);
  RCLCPP_INFO(get_logger(), " * Debug Body Tracking: %s", mDebugBodyTrk ? "TRUE" : "FALSE");
...
void ZedCamera::getSimParams() {
  ...
  if (mSimMode) {
    ...
    RCLCPP_WARN(get_logger(),
                "The parameter 'use_sim_time' is set to 'true', but simulation mode is not "
                "enabled. UNPREDICTABLE BEHAVIORS EXPECTED.");
  }
}
...
void ZedCamera::getGeneralParams() {
  ...
  if (camera_model == "zed") {
    ...
    if (mSvoMode) {
      RCLCPP_INFO_STREAM(get_logger(), " + Playing an SVO for " << sl::toString(mCamUserModel)
                                                                << " camera model.");
    } else if (mSimMode) {
      RCLCPP_INFO_STREAM(get_logger(),
                         " + Simulating a " << sl::toString(mCamUserModel) << " camera model.");
    } else if (!IS_JETSON) {
      RCLCPP_ERROR_STREAM(get_logger(), "Camera model "
  ...
  if (mSimMode) {
    ...
    if (resol == "AUTO") {
      ...
      RCLCPP_INFO_STREAM(get_logger(), " * Camera resolution: " << sl::toString(mCamResol).c_str());
    } else {
      if (resol == "HD2K") {
  ...
  if (mPubResolution == PubRes::CUSTOM) {
    getParam("general.pub_downscale_factor", mCustomDownscaleFactor, mCustomDownscaleFactor,
             " * Publishing downscale factor: ");
  } else {
    mCustomDownscaleFactor = 1.0;
  }

  getParam("general.optional_opencv_calibration_file", mOpencvCalibFile, mOpencvCalibFile,
...
void ZedCamera::getVideoParams() {
  ...
  if (sl_tools::isZEDX(mCamUserModel)) {
    ...
    getParam("video.exposure_compensation", mGmslExposureComp, mGmslExposureComp,
             " * [DYN] ZED X Exposure comp.: ", true);
    getParam("video.analog_gain", mGmslAnalogGain, mGmslAnalogGain,
             " * [DYN] ZED X Analog Gain: ", true);
    getParam("video.auto_analog_gain_range_min", mGmslAnalogGainRangeMin, mGmslAnalogGainRangeMin,
             " * [DYN] ZED X Auto Analog Gain range min: ", true);
    getParam("video.auto_analog_gain_range_max", mGmslAnalogGainRangeMax, mGmslAnalogGainRangeMax,
  ...
  if (get_parameter(paramName, paramVal)) {
    ...
    RCLCPP_WARN(get_logger(), "The parameter '%s' is not available, using the default value",
                paramName.c_str());
  }
...
void ZedCamera::getDepthParams() {
  ...
  if (!matched) {
    RCLCPP_WARN(get_logger(),
                "The parameter 'depth.depth_mode' contains a not valid string. Please check it in "
                "'common.yaml'.");
  ...
  if (mDepthMode == sl::DEPTH_MODE::NONE) {
    ...
    RCLCPP_INFO_STREAM(
        get_logger(), " * Depth mode: " << sl::toString(mDepthMode).c_str() << " - DEPTH DISABLED");
  } else {
    mDepthDisabled = false;
    RCLCPP_INFO_STREAM(get_logger(), " * Depth mode: " << sl::toString(mDepthMode).c_str() << " ["
  ...
  if (!mDepthDisabled) {
    ...
    if (mDepthStabilization < 0 || mDepthStabilization > 100) {
      ...
      RCLCPP_WARN_STREAM(get_logger(),
                         "'depth.depth_stabilization' is not in the valid range [0,100]. Using the "
                         "default value.");
    }
...
void ZedCamera::getMappingParams() {
  ...
  getParam("mapping.pd_max_distance_threshold", mPdMaxDistanceThreshold, mPdMaxDistanceThreshold,
           " * Plane Det. Max Dist. Thresh.: ");
  getParam("mapping.pd_normal_similarity_threshold", mPdNormalSimilarityThreshold,
...
void ZedCamera::getPosTrackingParams() {
  ...
  if (pos_trk_mode == "QUALITY") {
    ...
    RCLCPP_WARN_STREAM(get_logger(), "'pos_tracking.pos_tracking_mode' not valid ('"
                                         << pos_trk_mode << "'). Using default value.");
    mPosTrkMode = sl::POSITIONAL_TRACKING_MODE::QUALITY;
  }
  RCLCPP_INFO_STREAM(get_logger(),
                     " * Positional tracking mode: " << sl::toString(mPosTrkMode).c_str());
  ...
  RCLCPP_INFO(get_logger(), " * Initial pose: [%g,%g,%g,%g,%g,%g,]", mInitialBasePose[0],
              mInitialBasePose[1], mInitialBasePose[2], mInitialBasePose[3], mInitialBasePose[4],
              mInitialBasePose[5]);
  ...
  if (mGnssFusionEnabled) {
    ...
    RCLCPP_INFO(get_logger(),
                "   Note: loop closure (Area Memory) is disabled when using GNSS fusion");
  } else {
    ...
    getParam("pos_tracking.area_memory_db_path", mAreaMemoryDbPath, mAreaMemoryDbPath,
             " * Area Memory DB: ");
  }
  getParam("pos_tracking.set_as_static", mSetAsStatic, mSetAsStatic);
  RCLCPP_INFO_STREAM(get_logger(), " * Camera is static: " << (mSetAsStatic ? "TRUE" : "FALSE"));
  getParam("pos_tracking.set_gravity_as_origin", mSetGravityAsOrigin, mSetGravityAsOrigin);
  RCLCPP_INFO_STREAM(get_logger(), " * Gravity as origin [not for ZED]: "
  ...
  RCLCPP_INFO_STREAM(get_logger(), " * Init Odometry with first valid pose data: "
...
void ZedCamera::getGnssFusionParams() {
  ...
  if (mGnssFusionEnabled) {
...
#if (ZED_SDK_MINOR_VERSION == 0 && ZED_SDK_PATCH_VERSION < 6)
    getParam("gnss_fusion.gnss_init_distance", mGnssInitDistance, mGnssInitDistance,
             " * GNSS init. distance: ");
    ...
    RCLCPP_INFO_STREAM(get_logger(), " * GNSS Reinitialization: "
                                         << (mGnssEnableReinitialization ? "TRUE" : "FALSE"));
    getParam("gnss_fusion.enable_rolling_calibration", mGnssEnableRollingCalibration,
             mGnssEnableRollingCalibration);
    RCLCPP_INFO_STREAM(get_logger(), " * GNSS Rolling Calibration: "
                                         << (mGnssEnableRollingCalibration ? "TRUE" : "FALSE"));
    getParam("gnss_fusion.enable_translation_uncertainty_target",
             mGnssEnableTranslationUncertaintyTarget, mGnssEnableTranslationUncertaintyTarget);
    RCLCPP_INFO_STREAM(get_logger(),
    ...
    getParam("gnss_fusion.target_translation_uncertainty", mGnssTargetTranslationUncertainty,
             mGnssTargetTranslationUncertainty, " * GNSS Target Transl. Uncert.: ");
    ...
    RCLCPP_INFO_STREAM(get_logger(), " * Publish UTM TF: " << (mPublishUtmTf ? "TRUE" : "FALSE"));
...
void ZedCamera::getOdParams() {
  ...
  RCLCPP_INFO_STREAM(get_logger(),
                     " * Object Det. enabled: " << (mObjDetEnabled ? "TRUE" : "FALSE"));
  ...
  RCLCPP_INFO_STREAM(get_logger(), " * Object Det. model: " << sl::toString(mObjDetModel).c_str());

  getParam("object_detection.allow_reduced_precision_inference", mObjDetReducedPrecision,
           mObjDetReducedPrecision);
  RCLCPP_INFO_STREAM(get_logger(), " * Object Det. allow reduced precision: "
  ...
  getParam("object_detection.confidence_threshold", mObjDetConfidence, mObjDetConfidence,
  ...
  RCLCPP_INFO_STREAM(get_logger(),
                     " * MultiClassBox vehicles: " << (mObjDetVehiclesEnable ? "TRUE" : "FALSE"));
...
void ZedCamera::getBodyTrkParams() {
  ...
  RCLCPP_INFO_STREAM(get_logger(),
                     " * Body Track. enabled: " << (mBodyTrkEnabled ? "TRUE" : "FALSE"));
  ...
  RCLCPP_INFO_STREAM(get_logger(), " * Body Track. model: " << sl::toString(mBodyTrkModel).c_str());
  ...
  for (int idx = static_cast<int>(sl::BODY_FORMAT::BODY_18);
    ...
    std::replace(test_fmt_str.begin(), test_fmt_str.end(), ' ',
                 '_');  // Replace spaces with underscores to match the YAML setting
  ...
  RCLCPP_INFO_STREAM(get_logger(), " * Body Track. format: " << sl::toString(mBodyTrkFmt).c_str());

  getParam("body_tracking.allow_reduced_precision_inference", mBodyTrkReducedPrecision,
           mBodyTrkReducedPrecision);
  RCLCPP_INFO_STREAM(get_logger(), " * Body Track. allow reduced precision: "
                                       << (mObjDetReducedPrecision ? "TRUE" : "FALSE"));
  ...
  for (int idx = static_cast<int>(sl::BODY_KEYPOINTS_SELECTION::FULL);
    ...
    std::replace(test_body_sel_str.begin(), test_body_sel_str.end(), ' ',
                 '_');  // Replace spaces with underscores to match the YAML setting
  ...
  if (!matched) {
    RCLCPP_WARN_STREAM(
        get_logger(), "The value of the parameter 'body_tracking.body_kp_selection' is not valid: '"
                          << body_sel_str << "'. Using the default value.");
  }

  getParam("body_tracking.enable_body_fitting", mBodyTrkFitting, mBodyTrkFitting);
  RCLCPP_INFO_STREAM(get_logger(), " * Body fitting: " << (mBodyTrkFitting ? "TRUE" : "FALSE"));

  getParam("body_tracking.enable_tracking", mBodyTrkEnableTracking, mBodyTrkEnableTracking);
  RCLCPP_INFO_STREAM(get_logger(),
                     " * Body joints tracking: " << (mBodyTrkEnableTracking ? "TRUE" : "FALSE"));

  getParam("body_tracking.prediction_timeout_s", mBodyTrkPredTimeout, mBodyTrkPredTimeout,
           " * Body Track. prediction timeout [sec]: ");

  getParam("body_tracking.confidence_threshold", mBodyTrkConfThresh, mBodyTrkConfThresh,
           " * Body Track. confidence thresh.: ", true);

  getParam("body_tracking.minimum_keypoints_threshold", mBodyTrkMinKp, mBodyTrkMinKp,
...
void ZedCamera::getAdvancedParams() {
  ...
  getParam("advanced.thread_sched_policy", mThreadSchedPolicy, mThreadSchedPolicy,
           " * Thread sched. policy: ");
  ...
  if (mThreadSchedPolicy == "SCHED_FIFO" || mThreadSchedPolicy == "SCHED_RR") {
    if (!sl_tools::checkRoot()) {
      ...
      getParam("advanced.thread_grab_priority", mThreadPrioGrab, mThreadPrioGrab,
               " * Grab thread priority: ");
      getParam("advanced.thread_sensor_priority", mThreadPrioSens, mThreadPrioSens,
               " * Sensors thread priority: ");
      getParam("advanced.thread_pointcloud_priority", mThreadPrioPointCloud, mThreadPrioPointCloud,
...
rcl_interfaces::msg::SetParametersResult ZedCamera::callback_paramChange(
  ...
  for (const rclcpp::Parameter &param : parameters) {
    ...
    if (sl_tools::isZEDX(mCamRealModel)) {
      if (param.get_name() == "video.exposure_time") {
        ...
        RCLCPP_INFO_STREAM(get_logger(),
                           "Parameter '" << param.get_name() << "' correctly set to " << val);
        ...
        if ((val < mGmslAutoExpTimeRangeMin) || (val > max_val)) {
          ...
          result.reason =
              ...
              "[auto_exposure_time_range_min,max(30000,'general.grab_frame_rate'x1000)]";
          RCLCPP_WARN_STREAM(get_logger(), result.reason);
          break;
        }
        ...
        if ((val < 1000) || (val > 16000)) {
          result.successful = false;
          result.reason =
              param.get_name() + " must be a positive integer in the range [1000,16000]";
          RCLCPP_WARN_STREAM(get_logger(), result.reason);
          break;
        }
    ...
    if (param.get_name() == "general.pub_frame_rate") {
      ...
      if (sl_tools::isZEDX(mCamRealModel)) {
        RCLCPP_WARN_STREAM(get_logger(), "Parameter '" << param.get_name() << "' not available for "
      ...
      RCLCPP_INFO_STREAM(get_logger(), "Parameter '"
      ...
      if ((val < 0) || (val > 70)) {
        result.successful = false;
        result.reason = param.get_name() + " must be positive double value in the range [0,70]";
        RCLCPP_WARN_STREAM(get_logger(), result.reason);
        break;
      }
  ...
  if (result.successful) {
    RCLCPP_INFO_STREAM(get_logger(),
                       "Correctly set " << count << "/" << parameters.size() << " parameters");
  } else {
    RCLCPP_INFO_STREAM(get_logger(),
                       "Correctly set " << count - 1 << "/" << parameters.size() << " parameters");
  }
...
void ZedCamera::fillCamInfo(sl::Camera &zed,
...
void ZedCamera::initPublishers() {
  ...
  mPubRgbGray = image_transport::create_camera_publisher(this, rgb_gray_topic,
  ...
  mPubRawRgb = image_transport::create_camera_publisher(this, rgb_raw_topic,
                                                        mVideoQos.get_rmw_qos_profile());
  ...
  mPubRawRgbGray = image_transport::create_camera_publisher(this, rgb_raw_gray_topic,
                                                            mVideoQos.get_rmw_qos_profile());
  ...
  mPubLeftGray = image_transport::create_camera_publisher(this, left_gray_topic,
                                                          mVideoQos.get_rmw_qos_profile());
  ...
  mPubRawLeft = image_transport::create_camera_publisher(this, left_raw_topic,
                                                         mVideoQos.get_rmw_qos_profile());
  ...
  mPubRawLeftGray = image_transport::create_camera_publisher(this, left_raw_gray_topic,
                                                             mVideoQos.get_rmw_qos_profile());
  ...
  mPubRightGray = image_transport::create_camera_publisher(this, right_gray_topic,
                                                           mVideoQos.get_rmw_qos_profile());
  ...
  mPubRawRight = image_transport::create_camera_publisher(this, right_raw_topic,
                                                          mVideoQos.get_rmw_qos_profile());
  ...
  mPubRawRightGray = image_transport::create_camera_publisher(this, right_raw_gray_topic,
                                                              mVideoQos.get_rmw_qos_profile());
  ...
  if (!mDepthDisabled) {
    ...
    mPubPoseStatus =
        create_publisher<zed_interfaces::msg::PosTrackStatus>(mPoseStatusTopic, mPoseQos);
    ...
    mPubOdomStatus =
        create_publisher<zed_interfaces::msg::PosTrackStatus>(mOdomStatusTopic, mPoseQos);
    ...
    if (mGnssFusionEnabled) {
      ...
      RCLCPP_INFO_STREAM(get_logger(),
                         "Advertised on topic (GNSS): " << mPubGnssPose->get_topic_name());
      mPubGnssPoseStatus =
          create_publisher<zed_interfaces::msg::PosTrackStatus>(mGnssPoseStatusTopic, mPoseQos);
      RCLCPP_INFO_STREAM(get_logger(),
      ...
      RCLCPP_INFO_STREAM(get_logger(),
                         "Advertised on topic (GNSS): " << mPubGeoPose->get_topic_name());
      mPubGeoPoseStatus =
          create_publisher<zed_interfaces::msg::PosTrackStatus>(mGeoPoseStatusTopic, mPoseQos);
      RCLCPP_INFO_STREAM(get_logger(),
                         "Advertised on topic (GNSS): " << mPubGeoPoseStatus->get_topic_name());
    }
  }
  // <---- Pos Tracking
...
bool ZedCamera::startCamera() {
  ...
  // ----> SDK version
  RCLCPP_INFO(get_logger(), "ZED SDK Version: %d.%d.%d - Build %s", ZED_SDK_MAJOR_VERSION,
              ZED_SDK_MINOR_VERSION, ZED_SDK_PATCH_VERSION, ZED_SDK_BUILD_ID);
  // <---- SDK version
  ...
  mInitParams.async_grab_camera_recovery =
      true;  // Camera recovery is handled asynchronously to provide information about this status
  // <---- ZED configuration
  ...
  while (1) {
    ...
    if (mConnStatus == sl::ERROR_CODE::INVALID_CALIBRATION_FILE) {
      if (mOpencvCalibFile.empty()) {
        ...
        RCLCPP_ERROR_STREAM(get_logger(),
                            "Please check the correctness of the path of the calibration file in "
                            "the parameter 'general.optional_opencv_calibration_file': '"
                                << mOpencvCalibFile << "'.");
    ...
    if (mSvoMode) {
      ...
      if (mConnStatus == sl::ERROR_CODE::CAMERA_DETECTION_ISSUE &&
        ...
        RCLCPP_INFO(get_logger(),
                    "Try to flip the USB3 Type-C connector and verify the USB3 connection");
      } else {
  ...
  if (realFps != static_cast<float>(mCamGrabFrameRate)) {
    RCLCPP_WARN_STREAM(get_logger(), "!!! `general.grab_frame_rate` value is not valid: '"
                                         << mCamGrabFrameRate << "'. Automatically replaced with '"
                                         << realFps << "'. Please fix the parameter !!!");
...
#if (ZED_SDK_MINOR_VERSION == 0 && ZED_SDK_PATCH_VERSION >= 6)
  RCLCPP_INFO_STREAM(get_logger(), " * Focal Lenght -> "
  ...
  if (mSvoMode) {
    ...
    RCLCPP_INFO_STREAM(get_logger(), " * SVO framerate\t-> "
  ...
  RCLCPP_INFO_STREAM(get_logger(),
                     " * Camera grab frame size -> " << mCamWidth << "x" << mCamHeight);
  ...
  if (pub_w > mCamWidth || pub_h > mCamHeight) {
    RCLCPP_WARN_STREAM(get_logger(), "The publishing resolution ("
                                         << pub_w << "x" << pub_h
  ...
  RCLCPP_INFO_STREAM(get_logger(),
                     " * Publishing frame size  -> " << mMatResol.width << "x" << mMatResol.height);
  // <---- Camera information
  ...
  if (!mRoiParam.empty()) {
    ...
    if (!sl_tools::generateROI(sl_poly, roi_mask)) {
      ...
      if (err != sl::ERROR_CODE::SUCCESS) {
        RCLCPP_WARN_STREAM(get_logger(), " * Error while setting ZED SDK region of interest: "
  ...
  if (mDebugCamCtrl) {
    ...
    if (sl_tools::isZEDX(mCamRealModel)) {
      ...
      DEBUG_STREAM_CTRL("[ZEDX] Default value for " << sl::toString(setting).c_str() << ": "
                                                    << value);
      ...
      // if(err!=sl::ERROR_CODE::SUCCESS) {
      //   RCLCPP_ERROR_STREAM( get_logger(), "Error Getting default param for " <<
      //   sl::toString(setting).c_str() << ": " << sl::toString(err).c_str()); exit(EXIT_FAILURE);
      // }
      // DEBUG_STREAM_CTRL("[ZEDX] Default value for " << sl::toString(setting).c_str() << ": [" <<
      // value_min << "," << value_max << "]");
  ...
  if (mGnssFusionEnabled) {
    ...
    fus_err =
        mFusion.subscribe(mCamUuid, mFusionConfig->communication_parameters, mFusionConfig->pose);
    if (fus_err != sl::FUSION_ERROR_CODE::SUCCESS) {
      RCLCPP_ERROR_STREAM(
...
void ZedCamera::startTempPubTimer() {
  ...
  mTempPubTimer =
      create_wall_timer(std::chrono::duration_cast<std::chrono::milliseconds>(pubPeriod_msec),
...
bool ZedCamera::startPosTracking() {
  ...
  if (transformOk) {
    DEBUG_STREAM_PT("Time required to get valid static transforms: " << elapsed / 1000. << " sec");
  }

  RCLCPP_INFO(get_logger(), "Initial ZED left camera pose (ZED pos. tracking): ");
  RCLCPP_INFO(get_logger(), " * T: [%g,%g,%g]", mInitialPoseSl.getTranslation().x,
  ...
  if (mAreaMemoryDbPath != "" && !sl_tools::file_exist(mAreaMemoryDbPath)) {
    mAreaMemoryDbPath = "";
    RCLCPP_WARN_STREAM(get_logger(), "'area_memory_db_path' path doesn't exist or is unreachable: "
                                         << mAreaMemoryDbPath);
  }
...
bool ZedCamera::start3dMapping() {
  ...
  if (mMappingRes < lRes) {
    RCLCPP_WARN_STREAM(get_logger(), "'mapping.resolution' value ("
                                         << mMappingRes
  ...
  if (mMappingRangeMax < 0) {
    ...
    RCLCPP_INFO_STREAM(get_logger(), "Mapping: max range set to " << mMappingRangeMax
  ...
  if (err == sl::ERROR_CODE::SUCCESS) {
    ...
    RCLCPP_INFO_STREAM(get_logger(),
                       " * Map point cloud publishing rate: " << mFusedPcPubRate << " Hz");
...
bool ZedCamera::startObjDetect() {
  ...
  if (!sl_tools::isObjDetAvailable(mCamRealModel)) {
    RCLCPP_ERROR(get_logger(),
                 "Object detection not started. The camera model does not support it with the "
                 "current version "
                 "of the "
                 "SDK");
    return false;
...
bool ZedCamera::getCamera2BaseTransform() {
  DEBUG_STREAM_PT("Getting static TF from '" << mCameraFrameId.c_str() << "' to '"
                                             << mBaseFrameId.c_str() << "'");
  ...
  try {
    ...
    RCLCPP_INFO(get_logger(), "  * Translation: {%.3f,%.3f,%.3f}",
                mCamera2BaseTransf.getOrigin().x(), mCamera2BaseTransf.getOrigin().y(),
                mCamera2BaseTransf.getOrigin().z());
    RCLCPP_INFO(get_logger(), "  * Rotation: {%.3f,%.3f,%.3f}", roll * RAD2DEG, pitch * RAD2DEG,
                yaw * RAD2DEG);
    ...
    if (!mCamera2BaseFirstErr) {
      ...
      RCLCPP_WARN_THROTTLE(get_logger(), steady_clock, 1000.0,
                           "The tf from '%s' to '%s' is not available.", mCameraFrameId.c_str(),
                           mBaseFrameId.c_str());
      RCLCPP_WARN_THROTTLE(get_logger(), steady_clock, 1000.0,
...
bool ZedCamera::getSens2CameraTransform() {
  DEBUG_STREAM_PT("Getting static TF from '" << mDepthFrameId.c_str() << "' to '"
  ...
  try {
    ...
    RCLCPP_INFO(get_logger(), "  * Translation: {%.3f,%.3f,%.3f}",
                mSensor2CameraTransf.getOrigin().x(), mSensor2CameraTransf.getOrigin().y(),
    ...
    if (!mSensor2CameraTransfFirstErr) {
      ...
      RCLCPP_WARN_THROTTLE(get_logger(), steady_clock, 1000.0, "Transform error: %s", ex.what());
      RCLCPP_WARN_THROTTLE(get_logger(), steady_clock, 1000.0,
                           "The tf from '%s' to '%s' is not available.", mDepthFrameId.c_str(),
                           mCameraFrameId.c_str());
      RCLCPP_WARN_THROTTLE(get_logger(), steady_clock, 1000.0,
                           "Note: one of the possible cause of the problem is the absense of an "
                           "instance "
...
bool ZedCamera::getSens2BaseTransform() {
  ...
  try {
    ...
    RCLCPP_INFO(get_logger(), " Static transform Sensor to Base [%s -> %s]", mDepthFrameId.c_str(),
                mBaseFrameId.c_str());
    RCLCPP_INFO(get_logger(), "  * Translation: {%.3f,%.3f,%.3f}",
                mSensor2BaseTransf.getOrigin().x(), mSensor2BaseTransf.getOrigin().y(),
...
bool ZedCamera::getGnss2BaseTransform() {
  DEBUG_GNSS("Getting static TF from '%s' to '%s'", mGnssFrameId.c_str(), mBaseFrameId.c_str());
  ...
  try {
    ...
    mGnss2BaseTransf.setOrigin(tf2::Vector3(in.translation.x, in.translation.y, in.translation.z));
    // w at the end in the constructor
    ...
    RCLCPP_INFO(get_logger(), " Static transform Sensor to Base [%s -> %s]", mGnssFrameId.c_str(),
                mBaseFrameId.c_str());
    RCLCPP_INFO(get_logger(), "  * Translation: {%.3f,%.3f,%.3f}", mGnss2BaseTransf.getOrigin().x(),
    ...
    if (!mGnss2BaseTransfFirstErr) {
      ...
      RCLCPP_WARN_THROTTLE(get_logger(), steady_clock, 1000.0,
                           "The tf from '%s' to '%s' is not available.", mGnssFrameId.c_str(),
                           mBaseFrameId.c_str());
      RCLCPP_WARN_THROTTLE(get_logger(), steady_clock, 1000.0,
                           "Note: one of the possible cause of the problem is the absense of an "
                           "instance "
...
void ZedCamera::threadFunc_zedGrab() {
  ...
  if (mDebugAdvanced) {
    ...
    if (pthread_getschedparam(pthread_self(), &policy, &par)) {
      ...
      DEBUG_STREAM_ADV(" * Default GRAB thread (#" << pthread_self() << ") settings - Policy: "
  ...
  if (mDebugAdvanced) {
    ...
    if (pthread_getschedparam(pthread_self(), &policy, &par)) {
      ...
      DEBUG_STREAM_ADV(" * New GRAB thread (#" << pthread_self() << ") settings - Policy: "
                                               << sl_tools::threadSched2Str(policy).c_str()
                                               << " - Priority: " << par.sched_priority);
    }
  }
    ...
    if (!mSvoPause) {
      ...
      if (mGrabStatus != sl::ERROR_CODE::SUCCESS) {
        if (mGrabStatus == sl::ERROR_CODE::CAMERA_REBOOTING) {
          ...
          RCLCPP_ERROR_STREAM(
              get_logger(),
              "Critical camera error: " << sl::toString(mGrabStatus).c_str() << ". Node stopped.");
          break;
        }
      }
      // <---- Grab errors?
...
rclcpp::Time ZedCamera::publishSensorsData(rclcpp::Time t) {
  ...
  int totSub =
      imu_SubNumber + imu_RawSubNumber + imu_TempSubNumber + imu_MagSubNumber + pressSubNumber;
...
void ZedCamera::threadFunc_pointcloudElab() {
  ...
  if (mDebugAdvanced) {
    ...
    if (pthread_getschedparam(pthread_self(), &policy, &par)) {
      ...
      DEBUG_STREAM_ADV(" * Default Point Cloud thread (#"
                       << pthread_self()
                       << ") settings - Policy: " << sl_tools::threadSched2Str(policy).c_str()
                       << " - Priority: " << par.sched_priority);
    }
  }
  ...
  if (mDebugAdvanced) {
    ...
    if (pthread_getschedparam(pthread_self(), &policy, &par)) {
      ...
      DEBUG_STREAM_ADV(" * New Point Cloud thread (#" << pthread_self() << ") settings - Policy: "
                                                      << sl_tools::threadSched2Str(policy).c_str()
                                                      << " - Priority: " << par.sched_priority);
    }
  }
...
void ZedCamera::threadFunc_pubSensorsData() {
  ...
  if (mDebugAdvanced) {
    ...
    if (pthread_getschedparam(pthread_self(), &policy, &par)) {
      ...
      DEBUG_STREAM_ADV(" * Default Sensors thread (#" << pthread_self() << ") settings - Policy: "
                                                      << sl_tools::threadSched2Str(policy).c_str()
                                                      << " - Priority: " << par.sched_priority);
    }
  }
  ...
  if (mDebugAdvanced) {
    ...
    if (pthread_getschedparam(pthread_self(), &policy, &par)) {
      ...
      DEBUG_STREAM_ADV(" * New Sensors thread (#" << pthread_self() << ") settings - Policy: "
                                                  << sl_tools::threadSched2Str(policy).c_str()
                                                  << " - Priority: " << par.sched_priority);
    }
  }
...
bool ZedCamera::areVideoDepthSubscribed() {
  ...
  return (mRgbSubnumber + mRgbRawSubnumber + mRgbGraySubnumber + mRgbGrayRawSubnumber +
          mLeftSubnumber + mLeftRawSubnumber + mLeftGraySubnumber + mLeftGrayRawSubnumber +
          mRightSubnumber + mRightRawSubnumber + mRightGraySubnumber + mRightGrayRawSubnumber +
          mStereoSubnumber + mStereoRawSubnumber + mDepthSubnumber + mConfMapSubnumber +
          mDisparitySubnumber + mDepthInfoSubnumber) > 0;
}
...
void ZedCamera::retrieveVideoDepth() {
  ...
  if (mRightRawSubnumber + mStereoRawSubnumber > 0) {
    retrieved |=
        sl::ERROR_CODE::SUCCESS ==
        mZed.retrieveImage(mMatRightRaw, sl::VIEW::RIGHT_UNRECTIFIED, sl::MEM::CPU, mMatResol);
  ...
  if (mRgbGrayRawSubnumber + mLeftGrayRawSubnumber > 0) {
    retrieved |= sl::ERROR_CODE::SUCCESS == mZed.retrieveImage(mMatLeftRawGray,
                                                               sl::VIEW::LEFT_UNRECTIFIED_GRAY,
  ...
  if (mRightGrayRawSubnumber > 0) {
    retrieved |= sl::ERROR_CODE::SUCCESS == mZed.retrieveImage(mMatRightRawGray,
...
void ZedCamera::publishVideoDepth(rclcpp::Time &out_pub_ts) {
  ...
  if (mRgbSubscribed && (mDepthSubnumber > 0 || mDepthInfoSubnumber > 0)) {
    ...
    if (mRgbSubscribed && (ts_rgb.data_ns != 0 && (ts_depth.data_ns != ts_rgb.data_ns))) {
      RCLCPP_WARN_STREAM(get_logger(), "!!!!! DEPTH/RGB ASYNC !!!!! - Delta: "
                                           ...
                                           << " sec");
      RCLCPP_WARN(get_logger(),
                  "NOTE: this should never happen, please contact the node maintainer in case you "
                  "get this warning.");
    }
  }
  // <---- Check RGB/Depth sync
  ...
  if (mLastTs_grab.data_ns != 0) {
    ...
    DEBUG_STREAM_VD("VIDEO/DEPTH PUB LAST PERIOD: " << period_sec << " sec @" << 1. / period_sec
                                                    << " Hz");
  ...
  if (mLeftRawSubnumber > 0) {
    ...
    publishImageWithInfo(mMatLeftRaw, mPubRawLeft, mLeftCamInfoRawMsg, mLeftCamOptFrameId,
                         out_pub_ts);
  }
  ...
  if (mLeftGraySubnumber > 0) {
    ...
    publishImageWithInfo(mMatLeftGray, mPubLeftGray, mLeftCamInfoMsg, mLeftCamOptFrameId,
                         out_pub_ts);
  }
  ...
  if (mLeftGrayRawSubnumber > 0) {
    ...
    publishImageWithInfo(mMatLeftRawGray, mPubRawLeftGray, mLeftCamInfoRawMsg, mLeftCamOptFrameId,
                         out_pub_ts);
  }
  if (mRgbGrayRawSubnumber > 0) {
    DEBUG_STREAM_VD("mRgbGrayRawSubnumber: " << mRgbGrayRawSubnumber);
    publishImageWithInfo(mMatLeftRawGray, mPubRawRgbGray, mRgbCamInfoRawMsg, mDepthOptFrameId,
                         out_pub_ts);
  }
  // ----> Publish the left_raw_gray=rgb_raw_gray image if someone has
  // subscribed to
  ...
  if (mRightRawSubnumber > 0) {
    ...
    publishImageWithInfo(mMatRightRaw, mPubRawRight, mRightCamInfoRawMsg, mRightCamOptFrameId,
                         out_pub_ts);
  }
  // <---- Publish the right raw image if someone has subscribed to
  ...
  if (mRightGraySubnumber > 0) {
    ...
    publishImageWithInfo(mMatRightGray, mPubRightGray, mRightCamInfoMsg, mRightCamOptFrameId,
                         out_pub_ts);
  }
  // <---- Publish the right gray image if someone has subscribed to
  ...
  if (mRightGrayRawSubnumber > 0) {
    ...
    publishImageWithInfo(mMatRightRawGray, mPubRawRightGray, mRightCamInfoRawMsg,
  ...
  if (mStereoRawSubnumber > 0) {
    ...
    auto combined = sl_tools::imagesToROSmsg(mMatLeftRaw, mMatRightRaw, mCameraFrameId, out_pub_ts);
...
void ZedCamera::publishImageWithInfo(sl::Mat &img, image_transport::CameraPublisher &pubImg,
...
void ZedCamera::processOdometry() {
  ...
  if (!mInitOdomWithPose) {
    ...
    if (mPosTrackingStatusCamera == sl::POSITIONAL_TRACKING_STATE::OK ||
      ...
      DEBUG_PT("+++ Odometry [%s -> %s] - {%.3f,%.3f,%.3f} {%.3f,%.3f,%.3f}", mOdomFrameId.c_str(),
               mBaseFrameId.c_str(), mOdom2BaseTransf.getOrigin().x(),
               mOdom2BaseTransf.getOrigin().y(), mOdom2BaseTransf.getOrigin().z(), roll * RAD2DEG,
    ...
    DEBUG_STREAM_THROTTLE_PT(5000.0,
  ...
  if (odomSub) {
    ...
    for (size_t i = 0; i < odomMsg->pose.covariance.size(); i++) {
      ...
      if (mTwoDMode) {
        ...
        } else if ((i >= 2 && i <= 4) || (i >= 8 && i <= 10) || (i >= 12 && i <= 13) ||
                   (i >= 15 && i <= 16) || (i >= 18 && i <= 20) || (i == 22) ||
                   (i >= 24 && i <= 27)) {
...
void ZedCamera::processPose() {
  ...
  DEBUG_PT("Sensor POSE %s- [%s -> %s]:\n%s}", mDebugGnss ? "(`sl::Fusion`) " : "",
           mLeftCamFrameId.c_str(), mMapFrameId.c_str(), mLastZedPose.pose_data.getInfos().c_str());
  ...
  if (mDebugGnss) {
    ...
    DEBUG_PT("Sensor POSE (`sl::Camera`) [%s -> %s]:\n%s", mLeftCamFrameId.c_str(),
             mMapFrameId.c_str(), camera_pose.pose_data.getInfos().c_str());
  }
  ...
  if (mPosTrackingStatusWorld == sl::POSITIONAL_TRACKING_STATE::OK ||
    ...
    DEBUG_PT("*** Base POSE [%s -> %s] - {%.3f,%.3f,%.3f} {%.3f,%.3f,%.3f}", mMapFrameId.c_str(),
    ...
    if (initOdom || mResetOdom) {
      ...
      RCLCPP_INFO(get_logger(),
                  ...
                  mOdomFrameId.c_str(), mBaseFrameId.c_str(), mOdom2BaseTransf.getOrigin().x(),
                  mOdom2BaseTransf.getOrigin().y(), mOdom2BaseTransf.getOrigin().z(),
      ...
      DEBUG_PT("+++ Diff [%s -> %s] - {%.3f,%.3f,%.3f} {%.3f,%.3f,%.3f}", mMapFrameId.c_str(),
               mOdomFrameId.c_str(), mMap2OdomTransf.getOrigin().x(),
               mMap2OdomTransf.getOrigin().y(), mMap2OdomTransf.getOrigin().z(), roll * RAD2DEG,
               pitch * RAD2DEG, yaw * RAD2DEG);
    }
...
void ZedCamera::processGeoPose() {
  ...
  mLastLatLongPose.setCoordinates(mLastGeoPose.latlng_coordinates.getLatitude(),
  ...
  DEBUG_GNSS(" * ECEF: %.6f m, %.6f m, %.6f m", mLastEcefPose.x, mLastEcefPose.y, mLastEcefPose.z);
  DEBUG_GNSS(" * Lat. Long.: %.6f°, %.6f°,%.3f m", mLastLatLongPose.getLatitude(false),
             mLastLatLongPose.getLongitude(false), mLastLatLongPose.getAltitude());
  DEBUG_GNSS(" * Heading: %.3f°", mLastHeading * RAD2DEG);
  DEBUG_GNSS(" * UTM: %.5f m, %.5f m, %.5f°, %s", mLastUtmPose.easting, mLastUtmPose.northing,
  ...
  if (!mGnssInitGood) {
    ...
    mMap2UtmTransf.setOrigin(tf2::Vector3(mLastUtmPose.easting, mLastUtmPose.northing,
    ...
    if (!mUtmAsParent) {
      ...
      RCLCPP_INFO(get_logger(), " Static transform UTM to MAP [%s -> %s]", mUtmFrameId.c_str(),
                  mMapFrameId.c_str());
      RCLCPP_INFO(get_logger(), "  * Translation: {%.3f,%.3f,%.3f}", map2utmInverse.getOrigin().x(),
                  map2utmInverse.getOrigin().y(), map2utmInverse.getOrigin().z());
      RCLCPP_INFO(get_logger(), "  * Rotation: {%.3f,%.3f,%.3f}", roll * RAD2DEG, pitch * RAD2DEG,
                  yaw * RAD2DEG);
    } else {
      double roll, pitch, yaw;
      ...
      RCLCPP_INFO(get_logger(), " Static transform MAP to UTM [%s -> %s]", mMapFrameId.c_str(),
                  mUtmFrameId.c_str());
      RCLCPP_INFO(get_logger(), "  * Translation: {%.3f,%.3f,%.3f}", mMap2UtmTransf.getOrigin().x(),
                  mMap2UtmTransf.getOrigin().y(), mMap2UtmTransf.getOrigin().z());
      RCLCPP_INFO(get_logger(), "  * Rotation: {%.3f,%.3f,%.3f}", roll * RAD2DEG, pitch * RAD2DEG,
                  yaw * RAD2DEG);
    }
  }
  ...
  if (mMap2UtmTransfValid && mPublishUtmTf) {
    ...
    transformStamped.transform =
        mUtmAsParent ? (tf2::toMsg(mMap2UtmTransf.inverse())) : (tf2::toMsg(mMap2UtmTransf));
...
void ZedCamera::processDetectedObjects(rclcpp::Time t) {
  ...
  for (auto data : objects.object_list) {
    ...
    memcpy(&(objMsg->objects[idx].position_covariance[0]), &(data.position_covariance[0]),
           6 * sizeof(float));
    ...
    if (data.bounding_box_2d.size() == 4) {
      memcpy(&(objMsg->objects[idx].bounding_box_2d.corners[0]), &(data.bounding_box_2d[0]),
             8 * sizeof(unsigned int));
    }
    if (data.bounding_box.size() == 8) {
      memcpy(&(objMsg->objects[idx].bounding_box_3d.corners[0]), &(data.bounding_box[0]),
             24 * sizeof(float));
    }
...
void ZedCamera::processBodies(rclcpp::Time t) {
  ...
  for (auto body : bodies.body_list) {
    ...
    memcpy(&(bodyMsg->objects[idx].position_covariance[0]), &(body.position_covariance[0]),
           6 * sizeof(float));
    ...
    if (body.bounding_box_2d.size() == 4) {
      memcpy(&(bodyMsg->objects[idx].bounding_box_2d.corners[0]), &(body.bounding_box_2d[0]),
             8 * sizeof(unsigned int));
    }
    DEBUG_BT(" - BBox 2D OK");
    if (body.bounding_box.size() == 8) {
      memcpy(&(bodyMsg->objects[idx].bounding_box_3d.corners[0]), &(body.bounding_box[0]),
             24 * sizeof(float));
    }
    DEBUG_BT(" - BBox 3D OK");
    ...
    if (body.head_bounding_box.size() == 8) {
      memcpy(&(bodyMsg->objects[idx].head_bounding_box_3d.corners[0]), &(body.head_bounding_box[0]),
             24 * sizeof(float));
    }
    ...
    if (kp_size <= 70) {
      memcpy(&(bodyMsg->objects[idx].skeleton_2d.keypoints[0]), &(body.keypoint_2d[0]),
             2 * kp_size * sizeof(float));

      memcpy(&(bodyMsg->objects[idx].skeleton_3d.keypoints[0]), &(body.keypoint[0]),
...
void ZedCamera::applyVideoSettings() {
  ...
  if (!mSvoMode && mFrameCount % 5 == 0) {
    ...
    if (mTriggerAutoExpGain) {
      ...
      if (err != sl::ERROR_CODE::SUCCESS) {
        RCLCPP_WARN_STREAM(get_logger(), "Error setting AEC_AGC: " << sl::toString(err).c_str());
      } else {
    ...
    if (!mCamAutoExpGain) {
      ...
      if (err != sl::ERROR_CODE::SUCCESS) {
        RCLCPP_WARN_STREAM(get_logger(), "Error setting " << sl::toString(setting).c_str() << ": "
                                                          << sl::toString(err).c_str());
      }
    ...
    if (sl_tools::isZEDX(mCamRealModel)) {
      ...
      // TODO(Walter) Enable when fixed in the SDK
      // err =
      //   mZed.getCameraSettings(sl::VIDEO_SETTINGS::AUTO_DIGITAL_GAIN_RANGE, value_min,
      //   value_max);
      // if (err == sl::ERROR_CODE::SUCCESS &&
...
void ZedCamera::callback_pubTemp() {
  ...
  if (sl_tools::isZED2OrZED2i(mCamRealModel)) {
    sens_data.temperature.get(sl::SensorsData::TemperatureData::SENSOR_LOCATION::ONBOARD_LEFT,
                              mTempLeft);
    sens_data.temperature.get(sl::SensorsData::TemperatureData::SENSOR_LOCATION::ONBOARD_RIGHT,
                              mTempRight);
  } else {
  ...
  sens_data.temperature.get(sl::SensorsData::TemperatureData::SENSOR_LOCATION::IMU, mTempImu);
  // <---- Always update temperature values for diagnostic
...
void ZedCamera::callback_setPose(const std::shared_ptr<rmw_request_id_t> request_header,
  ...
  RCLCPP_INFO_STREAM(get_logger(), "New pose: [" << req->pos[0] << "," << req->pos[1] << ","
                                                 << req->pos[2] << ", " << req->orient[0] << ","
                                                 << req->orient[1] << "," << req->orient[2] << "]");
...
void ZedCamera::callback_enableBodyTrk(const std::shared_ptr<rmw_request_id_t> request_header,
                                       const std::shared_ptr<std_srvs::srv::SetBool_Request> req,
                                       std::shared_ptr<std_srvs::srv::SetBool_Response> res) {
  (void)request_header;
...
void ZedCamera::callback_updateDiagnostic(diagnostic_updater::DiagnosticStatusWrapper &stat) {
  ...
  if (mGrabStatus == sl::ERROR_CODE::SUCCESS) {
    ...
    stat.addf("Capture", "Tot. Processing Time: %.6f sec (Max. %.3f sec)", frame_proc_sec,
              frame_grab_period);
    ...
    if (mSysOverloadCount >= 10) {
      stat.summary(diagnostic_msgs::msg::DiagnosticStatus::WARN,
                   "System overloaded. Consider reducing 'general.pub_frame_rate' or "
    ...
    if (isDepthRequired()) {
      ...
      if (mObjDetRunning) {
        if (mObjDetSubscribed) {
          ...
          stat.addf("Object detection", "Processing Time: %.3f sec (Max. %.3f sec)",
      ...
      if (mBodyTrkRunning) {
        if (mBodyTrkSubscribed) {
          ...
          stat.addf("Body Tracking", "Processing Time: %.3f sec (Max. %.3f sec)",
...
void ZedCamera::callback_gnssFix(const sensor_msgs::msg::NavSatFix::SharedPtr msg) {
  ...
  if (!mGnssPubCheckTimer) {
    ...
    mGnssPubCheckTimer =
        create_wall_timer(std::chrono::duration_cast<std::chrono::milliseconds>(gnssTimeout_msec),
  ...
  DEBUG_STREAM_GNSS("GNSS Ts: " << ts_gnss_part_sec / 1000000000 << " sec + " << ts_gnss_part_nsec
                                << " nsec = " << ts_gnss_nsec << " nsec fused");

  if (ts_gnss_nsec <= mLastTs_gnss_nsec) {
    DEBUG_GNSS("callback_gnssFix: data not valid (timestamp did not increment)");
    return;
  }
  ...
  if (!mGnssFixValid) {
    ...
    DEBUG_STREAM_GNSS(" * First valid datum - Lat: " << std::fixed << std::setprecision(9) << latit
                                                     << "° - Long: " << longit
                                                     << "° - Alt: " << altit << " m");
  }
  ...
  if (mZed.isOpened() && mZed.isPositionalTrackingEnabled()) {
    ...
    if (ingest_error == sl::FUSION_ERROR_CODE::SUCCESS) {
      DEBUG_STREAM_GNSS("Datum ingested - [" << mGnssTimestamp.nanoseconds() << " nsec] " << latit
                                             << "°," << longit << "° / " << altit << " m");
    } else {
      RCLCPP_ERROR_STREAM(get_logger(),
...
void ZedCamera::callback_clickedPoint(const geometry_msgs::msg::PointStamped::SharedPtr msg) {
  ...
  RCLCPP_INFO_STREAM(get_logger(),
                     "Clicked 3D point [X FW, Y LF, Z UP]: [" << X << "," << Y << "," << Z << "]");
  ...
  try {
    ...
    RCLCPP_INFO(get_logger(), "'%s' -> '%s': {%.3f,%.3f,%.3f} {%.3f,%.3f,%.3f,%.3f}",
    ...
    RCLCPP_INFO(get_logger(), "Point in camera coordinates [Z FW, X RG, Y DW]: {%.3f,%.3f,%.3f}",
                camX, camY, camZ);
  } catch (tf2::TransformException &ex) {
    rclcpp::Clock steady_clock(RCL_STEADY_TIME);
    RCLCPP_WARN_THROTTLE(get_logger(), steady_clock, 1000.0, "Transform error: %s", ex.what());
    RCLCPP_WARN_THROTTLE(get_logger(), steady_clock, 1000.0,
                         "The tf from '%s' to '%s' is not available.", msg->header.frame_id.c_str(),
  ...
  sl::CalibrationParameters zedParam;
  zedParam =
      mZed.getCameraInformation(mMatResol).camera_configuration.calibration_parameters;  // ok
  ...
  DEBUG_STREAM_MAP("Clicked point image coordinates: ["
                   << u << "," << v << "] - out_scale_factor: " << out_scale_factor);
  // <---- Project the point into 2D image coordinates
...
#endif
  if (err != sl::ERROR_CODE::SUCCESS) {
    RCLCPP_WARN(get_logger(), "Error extracting plane at point [%.3f,%.3f,%.3f]: %s", X, Y, Z,
                sl::toString(err).c_str());
    return;
  }
  ...
  DEBUG_MAP("Found plane at point [%.3f,%.3f,%.3f] -> Center: [%.3f,%.3f,%.3f], Dims: %.3fx%.3f", X,
            Y, Z, center.x, center.y, center.z, dims[0], dims[1]);
  // <---- Extract plane from clicked point
  ...
  if (markerSubNumber > 0) {
    ...
    static int hit_pt_id = 0;  // This ID must be unique in the same process. Thus it is good to
    ...
    // Set the frame ID and timestamp.  See the TF tutorials for information on these.
    static int plane_mesh_id = 0;  // This ID must be unique in the same process. Thus it is good to
                                   // keep it as a static variable
...
void ZedCamera::callback_setRoi(const std::shared_ptr<rmw_request_id_t> request_header,
  ...
  if (req->roi == "") {
    std::string err_msg =
        "Error while setting ZED SDK region of interest: a vector of normalized points describing "
        "a "
...
void ZedCamera::callback_toLL(const std::shared_ptr<rmw_request_id_t> request_header,
  ...
  RCLCPP_INFO(
      ...
      geo_pose.latlng_coordinates.getLongitude(false), geo_pose.latlng_coordinates.getAltitude());
}

void ZedCamera::callback_fromLL(const std::shared_ptr<rmw_request_id_t> request_header,
  ...
  sl::LatLng ll_pt;
  ll_pt.setCoordinates(req->ll_point.latitude, req->ll_point.longitude, req->ll_point.altitude,
                       false);
  ...
  RCLCPP_INFO(get_logger(),
              "* Converted the GeoPoint %.6f°,%.6f° / %.2f m to MAP point (%.2fm,%.2fm,%.2fm)",
...
void ZedCamera::callback_clock(const rosgraph_msgs::msg::Clock::SharedPtr msg) {
  ...
  try {
    ...
    RCLCPP_WARN_STREAM(get_logger(), "Error comparing clock messages: "
