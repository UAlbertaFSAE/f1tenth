...
class Line:
    ...
    def sample(self, s: float) -> tuple[float, float, float, float]:
        """Point and heading at arc length ``s`` along this segment."""
        x = self.start[0] + s * math.cos(self.heading)
        y = self.start[1] + s * math.sin(self.heading)
        return x, y, self.heading, 0.0

    def end_point(self) -> tuple[float, float]:
        """Point at the end of this segment."""
        x, y, _heading, _curvature = self.sample(self.length)
        return x, y

    def end_heading(self) -> float:
        """Heading at the end of this segment."""
        return self.heading


@dataclass
class Arc:
    """Circular path segment of constant radius."""

    center: tuple
    radius: float
    start_angle: float
    delta_angle: (
        float  # signed; total turn = |delta_angle|, sign = turn direction (+ccw/left)
    )
    ...
    def sample(self, s: float) -> tuple[float, float, float, float]:
        """Point, heading and curvature at arc length ``s`` along this arc."""
    ...
    def end_point(self) -> tuple[float, float]:
        """Point at the end of this arc."""
        return self.sample(self.length)[:2]

    def end_heading(self) -> float:
        """Heading at the end of this arc."""
        return self.sample(self.length)[2]
...
class TrackPath:
    ...
    def sample(self, s: float) -> tuple[float, float, float, float]:
        """Point, heading and curvature at arc length ``s`` along the path."""
        ...
        for i in range(len(self.segments) - 1, -1, -1):
            if s >= self.offsets[i]:
                idx = i
                break
        sampled: tuple[float, float, float, float] = self.segments[idx].sample(
            s - self.offsets[idx]
        )
        return sampled


def build_straight_path() -> TrackPath:
    """Path for the straight test track."""
    return TrackPath([Line(start=(0.0, 0.0), heading=0.0, length=60.0)])


def build_eight_path() -> TrackPath:
    """Path for the figure-eight test track."""
    lead_in = 10.0
    radius = 8.0
    start = (lead_in, 0.0)
    return TrackPath(
...
def build_curved_path() -> TrackPath:
    ...
    def add_corner(radius: float) -> None:
        ...
        seg = Arc(
            center=center, radius=radius, start_angle=start_angle, delta_angle=quarter
        )
        segments.append(seg)
        pos = seg.end_point()
...
@dataclass
class Station:
    """One cone gate: a centerline point with its blue and yellow cone positions."""
...
def spacing_for_curvature(curvature: float) -> float:
    """Gate spacing for a given curvature: tighter turns get denser cones."""
    spacing = BASE_SPACING / (1.0 + CURVATURE_SPACING_GAIN * abs(curvature))
    return max(MIN_SPACING, min(MAX_SPACING, spacing))


def make_station(path: TrackPath, s: float) -> Station:
    """Build one cone gate at arc length ``s`` along the path."""
    ...
    return Station(
        s=s, x=x, y=y, heading=heading, half_width=HALF_WIDTH, blue=left, yellow=right
    )


def build_stations(path: TrackPath, closed: bool) -> list:
    """Lay cone gates along the whole path."""
    stations = []
    s = 0.0
...
def write_csv(path: Path, stations: list) -> None:
    """Write stations out as an id,x,y,color cone CSV."""
    with path.open("w", encoding="utf-8", newline="") as csv_file:
        ...
        for station_id, station in enumerate(stations):
            writer.writerow(
                [
                    station_id,
                    round(station.blue[0], 3),
                    round(station.blue[1], 3),
                    "blue",
                ]
            )
            writer.writerow(
                [
                    station_id,
                    round(station.yellow[0], 3),
                    round(station.yellow[1], 3),
                    "yellow",
                ]
            )
...
def main() -> None:
    """Generate the bundled track CSVs."""
    ...
    for name, (builder, closed) in TRACKS.items():
        ...
        print(
            f"{name}: {len(stations)} stations ({2 * len(stations)} cone rows), "
