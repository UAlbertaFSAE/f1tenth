...
def _local_turn_radius(p_prev, p_curr, p_next):
    """Circumradius of the 3-point triangle, i.e. the local radius of
    curvature of the centerline through p_curr, plus which side is concave.
...
def _smooth(values, window, closed):
    """Simple moving-average smoothing so the offset profile transitions
    gradually instead of jumping between adjacent samples."""
    if window < 2 or len(values) < window:
        return values
    kernel = np.ones(window) / window
    pad = window // 2
    mode = "wrap" if closed else "edge"
    padded = np.pad(values, pad, mode=mode)
