...
Resampled to a fixed arc-length resolution so the centerline has uniform point
spacing.
"""
...
def _padded_points(points: Sequence[Sequence[float]], closed: bool) -> list:
...
def generate_centerline(
    ...
    n = len(points)
    if n < 2:
        short: np.ndarray = np.array(points, dtype=float).reshape(-1, 2)
        return short
...
def _resample_uniform(dense: np.ndarray, resolution: float, closed: bool) -> np.ndarray:
