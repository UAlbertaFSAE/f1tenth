...
from map_generator import cone_generator, csv_io, spline, track_generator
...
class MapGeneratorApp(tk.Tk):
    """Tk application for drawing a track and exporting it as a cone CSV."""

    def __init__(self) -> None:
        """Build the window, canvas and control panels."""
        super().__init__()
        ...
        self.pan_x = 150.0
        self.pan_y = 400.0
        self._pan_drag_origin: tuple[int, int, float, float] | None = None

        # imported-CSV preview state
        self.imported_pairs: list[dict] | None = None
    ...
    def _build_editor_frame(self) -> None:
        f = self.editor_frame
        toolbar = ttk.Frame(f)
        toolbar.pack(side=tk.TOP, fill=tk.X, padx=4, pady=4)

        ttk.Button(toolbar, text="Open CSV", command=self.on_open_csv).pack(
            side=tk.LEFT, padx=2
        )
        ttk.Button(toolbar, text="Export CSV", command=self.on_export_csv).pack(
            side=tk.LEFT, padx=2
        )
        ttk.Button(toolbar, text="Clear", command=self.on_clear).pack(
            side=tk.LEFT, padx=2
        )
        ttk.Button(toolbar, text="Undo", command=self.on_undo).pack(
            side=tk.LEFT, padx=2
        )
        ttk.Button(toolbar, text="Redo", command=self.on_redo).pack(
            side=tk.LEFT, padx=2
        )
        ...
        ttk.Entry(toolbar, textvariable=self.track_width, width=6).pack(
            side=tk.LEFT, padx=2
        )
        ttk.Label(toolbar, text="Cone Spacing (m):").pack(side=tk.LEFT)
        ttk.Entry(toolbar, textvariable=self.cone_spacing, width=6).pack(
            side=tk.LEFT, padx=2
        )
        ttk.Label(toolbar, text="Spline Res (m):").pack(side=tk.LEFT)
        ttk.Entry(toolbar, textvariable=self.spline_resolution, width=6).pack(
            side=tk.LEFT, padx=2
        )
        ttk.Checkbutton(
            toolbar,
        ...
        for var in (self.track_width, self.cone_spacing, self.spline_resolution):
        ...
        for label, value in [
            ...
            ttk.Radiobutton(toolbar, text=label, value=value, variable=self.mode).pack(
                side=tk.LEFT, padx=2
            )

        ttk.Button(toolbar, text="NEXT ->", command=self.on_next).pack(
        ...
        self.canvas.bind(
            "<MouseWheel>", lambda e: self._zoom(e, 1.1 if e.delta > 0 else 1 / 1.1)
        )
        self.canvas.bind("<Configure>", lambda e: self._redraw_editor())

        self.status_var = tk.StringVar(value="Length: 0.0 m   Cones: 0")
        ttk.Label(f, textvariable=self.status_var, anchor=tk.W).pack(
    ...
    def _build_preview_frame(self) -> None:
        ...
        toolbar = ttk.Frame(f)
        toolbar.pack(side=tk.TOP, fill=tk.X, padx=4, pady=4)
        ttk.Button(toolbar, text="<- Back", command=self.on_back_from_preview).pack(
            side=tk.LEFT, padx=2
        )
        ttk.Button(toolbar, text="Export CSV", command=self.on_export_csv).pack(
            side=tk.LEFT, padx=2
        ...
        ttk.Label(
            f, textvariable=self.preview_status_var, anchor=tk.W, justify=tk.LEFT
    ...
    def _build_import_frame(self) -> None:
        ...
        toolbar.pack(side=tk.TOP, fill=tk.X, padx=4, pady=4)
        ttk.Button(
            toolbar, text="<- Back to editor", command=self.on_back_from_import
        ).pack(side=tk.LEFT, padx=2)
        ttk.Label(toolbar, text="Imported CSV preview (read-only)").pack(
            side=tk.LEFT, padx=8
        )
        ...
        ttk.Label(f, textvariable=self.import_status_var, anchor=tk.W).pack(
            side=tk.BOTTOM, fill=tk.X, padx=6, pady=4
        )
    ...
    def p2m(self, px: float, py: float) -> tuple[float, float]:
    ...
    # ------------------------------------------------------ mouse events --
    def _nearest_point_idx(self, px: float, py: float) -> int | None:
        best_idx: int | None = None
        best_d: float = POINT_HIT_RADIUS_PX
    ...
    # -------------------------------------------------------- computation --
    def _compute_track(self) -> tuple[np.ndarray, np.ndarray, np.ndarray, list]:
        ...
        pairs = cone_generator.generate_cone_pairs(
            centerline, left, right, spacing, closed
        )
        return centerline, left, right, pairs

    # ------------------------------------------------------------ drawing --
    def _draw_grid(self, c: tk.Canvas, w: int, h: int) -> None:
        """Draw the meter grid, with spacing adapted to zoom.

        Aims for ~25-100px between lines, with the origin axes drawn heavier and
        grid lines labeled in meters.
        """
        target_px = 50.0
        ...
        while gx <= x1:
            ...
            c.create_line(
                ...
                px,
                h,
                fill="#4a5568" if is_axis else "#e0e0e0",
    ...
    def _redraw_editor(self) -> None:
        ...
        if len(self.control_points) >= 2:
            ...
            if len(left) >= 2:
                c.create_line(
                    *[v for p in left for v in self.m2p(*p)], fill="#2b6cb0", width=2
                )
            if len(right) >= 2:
                c.create_line(
                    *[v for p in right for v in self.m2p(*p)], fill="#d69e2e", width=2
                )
            if len(centerline) >= 2:
                c.create_line(
                    *[v for p in centerline for v in self.m2p(*p)],
                    fill="#a0aec0",
                    dash=(4, 3),
                )
        ...
        if len(self.control_points) >= 2:
            c.create_line(
                *[v for p in self.control_points for v in self.m2p(*p)],
                fill="#cbd5e0",
                dash=(2, 2),
            )
        for x, y in self.control_points:
            px, py = self.m2p(x, y)
            c.create_rectangle(
                px - 4, py - 4, px + 4, py + 4, fill="#2d3748", outline=""
            )
    ...
    def _redraw_preview(self) -> None:
        ...
        avg_spacing = (
            length / max(1, len(pairs) - (0 if self.closed.get() else 1))
            if len(pairs) > 1
            else 0.0
        )
    ...
    def _redraw_import(self) -> None:
        ...
        yellow_pts = [
            p["yellow"] for p in self.imported_pairs if p["yellow"] is not None
        ]
        centers = csv_io.reconstruct_centerline(self.imported_pairs)

        if len(blue_pts) >= 2:
            c.create_line(
                *[v for p in blue_pts for v in self.m2p(*p)], fill="#2b6cb0", width=2
            )
        if len(yellow_pts) >= 2:
            c.create_line(
                *[v for p in yellow_pts for v in self.m2p(*p)], fill="#d69e2e", width=2
            )
        if len(centers) >= 2:
            c.create_line(
                *[v for p in centers for v in self.m2p(*p)], fill="#a0aec0", dash=(4, 3)
            )
    ...
    def on_back_from_preview(self) -> None:
        """Return to editing from the preview stage."""
        self.preview_frame.pack_forget()
        self.editor_frame.pack(fill=tk.BOTH, expand=True)
        self._redraw_editor()

    def on_back_from_import(self) -> None:
        """Return to editing from the import stage."""
        self.import_frame.pack_forget()
        self.editor_frame.pack(fill=tk.BOTH, expand=True)
        self._redraw_editor()

    def on_export_csv(self) -> None:
        """Write the current track out as a cone CSV."""
        if len(self.control_points) < 2:
            messagebox.showinfo("Map Generator", "Draw at least 2 points first.")
            return
        ...
        messagebox.showinfo(
            "Map Generator", f"Exported {len(pairs) * 2} cones to:\n{path}"
        )

    def on_open_csv(self) -> None:
        """Load a cone CSV back into the editor."""
