...
def _arc_length_indices(centerline: np.ndarray, spacing: float, closed: bool) -> list:
    n = len(centerline)
    if n < 2:
        return []
    ...
    targets = (
        np.linspace(0.0, total, n_stations, endpoint=not closed)
        if closed
        else np.arange(0.0, total + 1e-9, spacing)
    )
