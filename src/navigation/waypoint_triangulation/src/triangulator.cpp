...
Triangulator::Triangulator() : Node("triangulator_node") {
  ...
  waypoint_publisher =
      create_publisher<geometry_msgs::msg::Point>(waypoint_topic, rclcpp::QoS(10));
  marker_publisher = create_publisher<visualization_msgs::msg::MarkerArray>(marker_topic,
                                                                             rclcpp::QoS(10));

  RCLCPP_INFO(this->get_logger(), "Starting up triangulator (Cones frame input)");
}
...
void Triangulator::read_cones(const rc_interfaces::msg::Cones::ConstSharedPtr cones_msg) {
  ...
  rc_interfaces::msg::Cones normalized_frame;
  normalized_frame.cones.reserve(cones_msg->cones.size());
  for (const auto& cone : cones_msg->cones) {
    rc_interfaces::msg::Cone normalized = cone;
    normalized.color = normalize_color(cone.color);
    if (!normalized.color.empty()) {
      normalized_frame.cones.push_back(normalized);
    }
  }

  const rc_interfaces::msg::Cones filtered_frame = filter_frame_cones(normalized_frame);
  ...
  for (const auto& cone : filtered_frame.cones) {
    const std::string key = make_cone_key(cone);
    const int observation_count = ++cone_observation_counts_[key];
    if (observation_count < kMinObservationsToAccept) {
      continue;
    }
  ...
  RCLCPP_INFO_THROTTLE(this->get_logger(), *this->get_clock(), 2000,
                       ...
                       accumulated_cones_.cones.size(), pair.left.size(), pair.right.size(),
                       triangles.size(), waypoints.size(), promoted_count);
}

bool Triangulator::is_same_cone(const rc_interfaces::msg::Cone &coneA,
                                const rc_interfaces::msg::Cone &coneB) const {
  constexpr float kPositionTolerance = 0.2F;
  return (std::abs(coneA.x - coneB.x) <= kPositionTolerance) &&
         (std::abs(coneA.y - coneB.y) <= kPositionTolerance) &&
...
std::string Triangulator::normalize_color(const std::string& raw_color) const {
  ...
  if (color.find("blue") != std::string::npos || color == "left") {
    return LEFT;
  }
...
std::string Triangulator::make_cone_key(const rc_interfaces::msg::Cone& cone) const {
  const std::string normalized_color = normalize_color(cone.color);
  if (normalized_color.empty()) {
    return "unknown";
  }

  const int qx = static_cast<int>(std::lround(static_cast<double>(cone.x) / kConeKeyResolution));
  const int qy = static_cast<int>(std::lround(static_cast<double>(cone.y) / kConeKeyResolution));
  return normalized_color + ":" + std::to_string(qx) + ":" + std::to_string(qy);
}
...
rc_interfaces::msg::Cones Triangulator::filter_frame_cones(
  ...
  for (const auto& cone : frame_cones.cones) {
    rc_interfaces::msg::Cone normalized = cone;
    normalized.color = normalize_color(cone.color);
    if (normalized.color == LEFT || normalized.color == RIGHT) {
  ...
  const double cluster_dist_sq = kClusterDistance * kClusterDistance;
  ...
  for (const auto& cone : cluster_cones) {
    ...
    if (nearest_opposite >= kMinTrackWidth && nearest_opposite <= kMaxTrackWidth) {
...
LR Triangulator::split(const rc_interfaces::msg::Cones& cones) {
  ...
  for (const auto& cone : cones.cones) {
    rc_interfaces::msg::Cone normalized = cone;
    normalized.color = normalize_color(cone.color);
    if (normalized.color == LEFT) {
      pair.left.push_back(normalized);
    } else if (normalized.color == RIGHT) {
...
std::vector<geometry_msgs::msg::Point> Triangulator::build_waypoints_from_triangulation(
  ...
  try {
    cdt.insertVertices(points.begin(), points.end(), [](const Point2D& p) { return p.x; },
                       [](const Point2D& p) { return p.y; });
    cdt.insertEdges(boundary_edges.begin(), boundary_edges.end(),
  ...
  std::sort(waypoints.begin(), waypoints.end(), [](const geometry_msgs::msg::Point& a,
...
std::vector<geometry_msgs::msg::Point> Triangulator::build_waypoints_from_pairs(
  ...
  for (const auto& left : pair.left) {
    ...
    for (const auto& right : pair.right) {
      ...
      if (dist < kMinTrackWidth || dist > kMaxTrackWidth) {
        continue;
      }
...
void Triangulator::publish_markers(
  ...
  for (const auto& cone : cones.cones) {
    ...
    const std::string normalized_color = normalize_color(cone.color);
    if (normalized_color == LEFT) {
      left_cones_marker.points.push_back(p);
    } else if (normalized_color == RIGHT) {
