...
def main() -> None:
    ...
    init_params.depth_mode = (
        sl.DEPTH_MODE.ULTRA
    )  # depth not strictly needed for 2D detect
        ...
        while True:
            ...
            try:
                results = model.predict(
                    frame_bgr, conf=0.5, device=device, verbose=False
                )
            except Exception as e:
                msg = str(e).lower()
                if "no kernel image" in msg or "cuda error" in msg:
                    print(
                        "CUDA execution failed; falling back to CPU for YOLO inference..."
                    )
                    device = "cpu"
                    with suppress(Exception):
                        model.to("cpu")
                    results = model.predict(
                        frame_bgr, conf=0.5, device="cpu", verbose=False
                    )
                else:
                    raise
