...
class ConePublisher(Node):
    def __init__(self):
        ...
        self.get_logger().info(f'Starting detection camera node')
        ...
        if torch is not None:
            try:
                ...
                self.cuda_available = False
                self.device = 'cpu'
        self.get_logger().info(f'Device selected: {self.device} (cuda_available={self.cuda_available})')

        # Publishers / subscribers
        self.publisher_ = self.create_publisher(ObjectsStamped, self.publishing_topic, 10)

        # Publishers for visualization
        if self.visualize:
            self.viz_image_pub = self.create_publisher(Image, '/detection_visualization/detections', 10)
            self.viz_depth_pub = self.create_publisher(Image, '/detection_visualization/depth', 10)
        ...
        if self.include_depth:
            ...
            self.caminfo_sub = self.create_subscription(
                ...
                10)
            self.get_logger().info(f'Subscribed to depth at: {self.depth_node}/depth/depth_registered')
            self.get_logger().info(f'Subscribed to camera info at: {self.camera_node}/depth/depth_registered/camera_info')

        self.get_logger().info(f'Subscribed to image at: {self.camera_node}/rgb/color/rect/image')

        # State
        self.bridge = CvBridge()
        self.model = self.load_model()
        self.classes = self.load_classes(self.classes_file)
        self.latest_depth = None          # numpy array (meters)
    ...
    def depth_callback(self, msg: Image):
        ...
        try:
            ...
            self.get_logger().error("Unexpected error converting depth image:\n" + traceback.format_exc())

    def caminfo_callback(self, msg: CameraInfo):
        """
        Store the latest CameraInfo (intrinsics). We'll use msg.k (or msg.K).
        """
    ...
    def left_image_callback(self, msg: Image):
        """
        Called for each incoming left rectified image.
        ...
        try:
            ...
            self.get_logger().error("Unexpected error converting left image:\n" + traceback.format_exc())
            return

        # Run YOLO (pass device to offload to GPU if available)
        try:
            results = self.model(cv_image, conf=self.detection_confidence, device=self.device, verbose=False)
        except Exception:
            self.get_logger().error("YOLO model run failed:\n" + traceback.format_exc())
            return

        # Build ObjectsStamped
        objects_list, detections_info = self.process_detections(results, cv_image.shape)
    ...
    def process_detections(self, results, image_shape):
        ...
        image_shape: (H, W, C)
        Returns: (cone_objects, detections_info) where detections_info is for visualization
        """
        cone_objects = []
        ...
        if self.include_depth and self.latest_caminfo is not None:
            try:
                K = self.latest_caminfo.k if hasattr(self.latest_caminfo, 'k') else self.latest_caminfo.K
                fx = float(K[0])
                fy = float(K[4])
                cx = float(K[2])
                cy = float(K[5])
            except Exception:
                self.get_logger().warn("Failed to read camera intrinsics from CameraInfo message.")
                fx = fy = cx = cy = None
            ...
            for box in result.boxes:
                try:
                    ...
                    try:
                        ...
                        self.get_logger().warn("Unexpected box format from YOLO; skipping box.")
                        continue

                obj = Object()

                # Label fields
                class_label = self.classes[class_id] if class_id < len(self.classes) else f"unknown_{class_id}"
                ...
                try:
                    ...
                    self.get_logger().warn("Could not populate BoundingBox2Di exactly; continuing with minimal bbox.")
                ...
                if self.include_depth and self.latest_depth is not None and fx and fy and not math.isnan(fx) and not math.isnan(fy):
                    ...
                    if x2_i <= x1_i or y2_i <= y1_i:
                        self.get_logger().warn("Invalid bbox size after clamping; skipping depth for this object.")
                    else:
                        ...
                        if depth_roi.size == 0:
                            ...
                            valid_mask = np.isfinite(depth_roi) & (depth_roi > 0.001)  # ignore zeros/near-zero
                            if np.any(valid_mask):
                                ...
                                var_z = (0.02 * Z) ** 2  # 2% relative depth noise -> variance
                                var_xy = ((px_width + px_height) / 2.0) * (Z / fx) * 0.005
    ...
    # Visualization
    # -------------------------
    def publish_visualization(self, cv_image, detections_info, header):
        """
