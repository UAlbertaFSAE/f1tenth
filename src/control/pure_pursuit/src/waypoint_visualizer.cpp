...
WaypointVisualizer::WaypointVisualizer() : Node("waypoint_visualizer_node") {
  ...
  vis_path_pub =
      this->create_publisher<visualization_msgs::msg::MarkerArray>(rviz_waypoints_topic, 1000);
...
void WaypointVisualizer::download_waypoints() {  // put all data in vectors
  ...
  RCLCPP_INFO(this->get_logger(), "%s",
              (csvFile_waypoints.is_open() ? "fileOpened" : "fileNOTopened"));
...
void WaypointVisualizer::timer_callback() { visualize_points(); }
